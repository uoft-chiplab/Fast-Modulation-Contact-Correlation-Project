...
for (scale, EF) in zip(scale_list, EF_list):
    ...
    dict_list.append({
        ...
        'trap_den_x_thermal_debrog': TUGs.psd_trap/TUGs.Ns,
        'Ns': TUGs.Ns
    })
...
# ax2.set_ylabel('Trap den × thermal debrog')
ax.set(
    xlabel = 'ODT Scale', 
    ylabel = rf'$\langle n\rangle$'
)
# ax2 = ax[0].twinx()
# ax2.plot(scale_list, therm_lamb)
# ax2.set(
#     ylim = [therm_lamb.min(), therm_lamb.max()]
# )
# ax[1].plot(scale_list, therm_lamb)
# ax[1].set(
#     ylim = [therm_lamb.min(),therm_lamb.max()]
# )
