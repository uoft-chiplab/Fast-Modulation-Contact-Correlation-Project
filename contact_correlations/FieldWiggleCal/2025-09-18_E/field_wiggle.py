...
file_list = glob.glob(f"{os.path.dirname(__file__)}\\*pre*.dat")
...
# convolve sine and sinc^2 fits
ts = np.linspace(np.min(fit_df['time']), np.max(fit_df['time']), 100)
sin_arr = Sin(ts, *popt)
sinc2_arr = ()
convolve(sin_arr, sinc2_arr)